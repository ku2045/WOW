...
def cars(request, pk):
    ...
    vehicles = random.choices(RrskVehicleClass.objects.raw(
        'SELECT a.id, b.id as vid, b.v_make, b.v_model, a.class_name, a.daily_rate, a.daily_mileage_limit, a.over_mileage_fee FROM rrsk_vehicle_class a JOIN rrsk_vehicle b ON a.id = b.v_class_id'),
                              k=20)
    print(vehicles)
...
def payOrder(request, pk):
    ...
    if request.method == 'POST':
        pay = RrskInvoicePayment(pay_amount=order.invoice_amount, pay_method=card_type, card_no=cardno,
                                 invoice_no=order)
        pay.save()
...
def generate_customers(num_customers=200):
    first_names = ['John', 'James', 'Hank', 'Tim', 'Joan', 'Jane', 'Willy', 'Paul', 'Sarah', 'Bonnie', 'Will', 'Arthur',
                   'Rudy', 'Tom', 'Liza', 'Lizze', 'Rose', 'Lily', 'Amanda']
    last_names = ['Smith', 'Stevens', 'Paulson', 'Cragson', 'Mansfield', 'Barmaeger', 'Wolls', 'Winner', 'Idleman',
                  'Shaper', 'Bolt', 'Doad']
...
def generate_vehicles(num=70):
    models = ['Range Rover', 'Fielder', 'Icemaker', 'Blastmaster', 'Top-Heavy', 'Miatta', 'Oboe', 'Beatle', 'Bug',
              'Land Cruiser', 'Big Bertha', '911 Turbo', 'Supercharger', 'Ace', 'MX123', 'Mandlevroch']
